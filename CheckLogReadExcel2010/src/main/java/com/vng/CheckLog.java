    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < vngGroups.length; i++) {
            try {
                ...
                for (int j = 1; j < logType.length + 1; j++) {
                    String type = (String) gameProperty.getProperty(logType[j-1] + ".pattern");
                    type = type.replace("*", ".*");
                    gameProperties[i][j] = type;
                }
            } catch (IOException e) {
                        ...
                        if (!server.contains(listOfServer[j].getName())) {
                            ...
                            for (int l = 1; l < gameProperties[i].length; l++) {
                                for (int k = 0; k < listOfLogs.length; k++) {
                                    if (listOfLogs[k].getName().matches(gameProperties[i][l])) {
                                        if (listOfLogs[k].length() / 1024 >= ccuFilesize) {
                                            count++;
                                            break;
                                        }
                                    }
